%% %% Calculating technical specification for cooling exchanger type tube in tube: Capacity, height, diameter,...
clc
clear
%% Nhiet do
t_nv=150;%nhiet do dong nong vao
t_nr=40;%nhiet do dong nong ra
t_lv=30;%nhiet do dong lanh vao
t_lr=80;%nhiet do dong lanh ra
...
dk_ngoai=100*10^(-3);%m
bd_ngoai=2*10^(-3);%m
F_nvao = 33.33; %luu luong dong lanh (L/ph)
%% Delta T
...
%% Tim
ro_nong=interp1(t_dau,ro_dau,ttb_nong);
cp_nong=interp1(t_dau,cp_dau,ttb_nong);
muy_nong=interp1(t_dau,muy_dau,ttb_nong)*10^(-3);
lamda_nong=interp1(t_dau,lamda_dau,ttb_nong);
...
%%Khoi luong rieng nuoc tr11ST1
if ttb_lanh<=50
    ro_lanh=interp1(t_ro1,ro1,ttb_lanh);
else
    ro_lanh=interp1(t_ro2,ro2,ttb_lanh);
end
%% ---------LAMDA------------
...
lamda_lanh=interp1(t_lamda,gtr_lamda,ttb_lanh)*1.163*10^(-2);%W/m.k trang133ST1
%% ----------MUY--------------------------
...
muy_lanh=interp1(t_muy,gtr_muy,ttb_lanh)*10^(-3);%N/s.m2 trang95ST1
%% -------------Cp-------------------
...
cp_lanh=interp1(t_cp,gtr_cp,ttb_lanh)*4.1868;%J/kg.k nhiet dung rieng nuoc nong tr165ST1
%% -------------efxilong------------
...
%-----------------------%
G_nong=(ro_nong*F_nvao*10^(-3))/60;%kg/s luu luong khoi luong
Q_nong=G_nong*cp_nong*(t_nv-t_nr);%W
Q_lanh =Q_nong - tt*Q_nong;%W
G_lanh=Q_lanh/(cp_lanh*(t_lr-t_lv));%kg/s luu luong khoi luong
F_lvao=(G_lanh/ro_lanh);%m3/s luu luong the tich
%-----------------------%
F_trong=(pi*d^2)/4;%m2 tiet dien ong trong
F_ngoai=(pi*D^2)/4-F_trong;%m2 tiet dien ong ngoai
w_nong=(F_nvao*10^(-3))/(F_trong*60);%m/s toc do dong nong
w_lanh=(F_lvao/F_ngoai);%m/s toc do dong lanh
%-----------------------%
Dtd_nong = d;%duong kinh tuong duong ong trong
Dtd_lanh =(D-dk_trong);%duong kinh tuong duong ong ngaoi
%-----------Re------------%
Re_nong =(w_nong*Dtd_nong*ro_nong)/muy_nong;
...
f = 0;
while true
    ro_tv1=interp1(t_dau,ro_dau,x);
    cp_tv1=interp1(t_dau,cp_dau,x);
    muy_tv1=interp1(t_dau,muy_dau,x)*10^(-3);
    lamda_tv1=interp1(t_dau,lamda_dau,x);
    ...
    q1=alpha1*(t_nv-x);
    %--------------tim tv2--------------
    tv2= x-((q1*bd_trong)/lamda_vach);%nuoc
    if tv2<=50
    ro_tv2=interp1(t_ro1,ro1,tv2);
    else
    ro_tv2=interp1(t_ro2,ro2,tv2);
    end
    lamda_tv2=interp1(t_lamda,gtr_lamda,tv2)*1.163*10^(-2);%W/m.k 
    muy_tv2=interp1(t_muy,gtr_muy,tv2)*10^(-3);%N/s.m2 
    Re_tv2 =(w_lanh*Dtd_lanh*ro_tv2)/muy_tv2;
    cp_tv2=interp1(t_cp,gtr_cp,tv2)*4.1868;%J/kg.k 
    %Prt=(cp_tv2*muy_tv2*ro_tv2)/lamda_tv2;
    Pr_tv2=(cp_tv2*muy_tv2*ro_tv2)/lamda_tv2;
    %Nu_tv2=0.23*Re_tv2^(0.8)*Pr_tv2^(0.4)*(Dtd_lanh/Dtd_nong)^0.45;
    Nu_tv2=0.021*efxilong_lanh*Re_tv2^(0.8)*Pr_tv2^(0.43)*(Pr_tv2/Pr_tv2)^0.25;%Re>10000
    alpha2=(Nu_tv2*lamda_tv2)/Dtd_lanh; %Nu=(a*dtd)/lamda tr11/ST2
    q2=alpha2*(tv2-t_lv);
    %so sanh
